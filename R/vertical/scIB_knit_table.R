...
scIB_knit_table <- function(
  ...
  colors <- NULL
  palette <- colorRampPalette(brewer.pal(9, palettes[["vertical.overall"]]))
  all_colour <- palette(200)  
  ...
  leg_min_x <- 5
  rank_groups <- c("vertical.overall", "vertical.metric")
  ...
  rank_minimum_x <- list("vertical.overall" = leg_min_x, 
                         "vertical.metric" = leg_min_x + 1)
