...
load_all_data_mosaic <- function(base_dir) {
...
generate_individual_bubble_mosaic <- function(ds, select_metric, select_method,  base_dir, task_category, new_file_paths=NA, new_file_names=NA) {

  if (task_category=="clustering"){source("./R/mosaic/scIB_knit_table_clustering.R")}
  if (task_category=="batchcorrection"){source("./R/mosaic/scIB_knit_table_batch.R")}
  if (task_category=="classification"){source("./R/mosaic/scIB_knit_table_classification.R")}
  combined_data <- load_all_data_mosaic(base_dir)
...
generate_summary_bubble_mosaic <- function(ds, select_metric, select_method,  base_dir,task_category) {
  source("./R/mosaic/mosaic_config.R")
  if (task_category=="clustering"){source("./R/mosaic/scIB_knit_table_clustering_summary.R")}
  if (task_category=="batchcorrection"){source("./R/mosaic/scIB_knit_table_batch_summary.R")}
  if (task_category=="classification"){source("./R/mosaic/scIB_knit_table_classification_summary.R")}
  combined_data <- load_all_data_mosaic(base_dir)
  combined_data <- combined_data %>%
    mutate(
      method = ifelse(grepl(".ori$", metric), paste0(method, ".ori"), method),
