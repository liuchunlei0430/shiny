...
scIB_knit_table <- function(
  ...
  colors <- NULL
  palette <- colorRampPalette(brewer.pal(9, palettes[["repro.overall"]]))
  all_colour <- palette(200)  
  ...
  text_data <- data.frame(label_value = as.vector(dat_mat), 
                          ...
                          size = 3.5, fontface = "plain", stringsAsFactors = F)
  ...
  leg_min_x <- 5
  rank_groups <- c("repro.overall", "repro.metric")
  ...
  rank_minimum_x <- list("repro.overall" = leg_min_x,
                         "repro.metric" = leg_min_x + 1)
