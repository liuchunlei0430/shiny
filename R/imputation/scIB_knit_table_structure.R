...
scIB_knit_table <- function(
  ...
  if(length(ind_circle) > 0){
    ...
    palette <- colorRampPalette(brewer.pal(9, "Purples"))
  ...
  colors <- NULL
  palette <- colorRampPalette(brewer.pal(9, palettes[["structure.overall"]]))
  all_colour <- palette(200)  
  ...
  leg_min_x <- 5
  rank_groups <- c("structure.overall", "structure.metric")
  ...
  rank_minimum_x <- list("structure.overall" = leg_min_x,
                         "structure.metric" = leg_min_x + 1)
  ...
  if (nrow(text_data) > 0) {
    ...
    text_data_left$size[which(text_data_left$label_value %in% colnames(data))] <- 4
