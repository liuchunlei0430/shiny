...
load_all_data_diagonal <- function(base_dir) {
  source("./R/diagonal/diagonal_config.R")
...
generate_individual_bubble_diagonal <- function(ds, select_metric, select_method,  base_dir, task_category,  new_file_paths=NA, new_file_names=NA) {

  if (task_category=="clustering"){source("./R/diagonal/scIB_knit_table_clustering.R")}
  if (task_category=="batchcorrection"){source("./R/diagonal/scIB_knit_table_batch.R")}
  if (task_category=="classification"){source("./R/diagonal/scIB_knit_table_classification.R")}
  combined_data <- load_all_data_diagonal(base_dir)
...
generate_summary_bubble_diagonal <- function(ds, select_metric, select_method,  base_dir,task_category) {

  if (task_category=="clustering"){source("./R/diagonal/scIB_knit_table_clustering_summary.R")}
  if (task_category=="batchcorrection"){source("./R/diagonal/scIB_knit_table_batch_summary.R")}
  if (task_category=="classification"){source("./R/diagonal/scIB_knit_table_classification_summary.R")}
  combined_data <- load_all_data_diagonal(base_dir)
