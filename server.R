...
server <- function(input, output, session) {
  ...
  filtered_data <- reactive({
    ...
    if (!is.null(input$search_text) && input$search_text != "") {
      ...
      data <- data[grepl(search_pattern, tolower(paste(data[,1], 
                                                      data$modality, 
                                                      data$protocol, 
                                                      data$description, 
                                                      sep = " "))), ]
  ...
  output$protocol_plot <- renderPlotly({
    ...
    plot_ly(df, labels = ~protocol, values = ~count, type = 'pie',
            textposition = 'inside',
            hoverinfo = 'text',
  ...
  method_data_processed <- reactive({
    ...
    # 提取所有可能的数据结构类型
    data_structures <- unique(unlist(lapply(data$Data.Stucture., function(x) {
      if (is.na(x)) return(character(0))
      safe_split(gsub("\\[|\\]", "", x))
    })))
    data_structures <- data_structures[data_structures != ""]

    # 提取Peak/Gene Activity选项
    peak_gene_opts <- unique(data$Peak.Gene.Activity)
    peak_gene_opts <- peak_gene_opts[!is.na(peak_gene_opts) & peak_gene_opts != ""]

    # 提取所有输出类型
    output_types <- unique(data$Output)
    output_types <- output_types[!is.na(output_types) & output_types != ""]
  ...
  filtered_method_data <- reactive({
    ...
    # 单选筛选条件
    if (!is.null(input$peak_gene) && input$peak_gene != "") {
        data <- data[data$Peak.Gene.Activity == input$peak_gene, ]
    }
    
    if (!is.null(input$output_type) && input$output_type != "") {
        data <- data[data$Output == input$output_type, ]
    }
    
    if (!is.null(input$prog_lang) && input$prog_lang != "") {
        data <- data[data$Programming.Language == input$prog_lang, ]
    }
    
    if (!is.null(input$deep_learning) && input$deep_learning != "") {
        data <- data[data$Deep.Learning == input$deep_learning, ]
    }
    
    if (!is.null(input$cell_type) && input$cell_type != "") {
